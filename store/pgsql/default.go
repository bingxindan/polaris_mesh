...
// Name 实现Name函数
func (p *pgsqlStore) Name() string {
	return STORENAME
}

// Initialize 初始化函数
func (p *pgsqlStore) Initialize(conf *store.Config) error {
	if p.start {
		return nil
	}
...
// Destroy 退出函数
func (p *pgsqlStore) Destroy() error {
	p.start = false
...
func (p *pgsqlStore) CreateTransaction() (store.Transaction, error) {
	// 每次创建事务前，还是需要ping一下
	_ = p.masterTx.Ping()
...
func (p *pgsqlStore) StartTx() (store.Tx, error) {
	tx, err := p.masterTx.Begin()
	if err != nil {
		return nil, err
	}
